...
Last Updated: 2026-08-12 (PR #256 opened — frame-delivery-pattern probe for GH #251, v0.1.940, diagnostics only, hardware validation pending; three recv-path follow-ups filed (#253/#254/#255); PR #236 (2026-08-11) still hardware-validation pending)
...
### 4. Latency Reduction Initiative *(Hardware A/B 2026-08-11 — PRs #213/#215/#223 Confirmed; #227 Pending; #228/#221 Benign. GH #251 (video delivery burstiness, 2026-08-12) narrowed to a measurement gap; PR #256 diagnostics probe opened, hardware validation pending. Open follow-ups #219/#220/#221/#222/#224/#225/#226/#253/#254/#255 filed.)*
...
- **Completed & Hardware Validated (PR #213):** Post-FEC effective loss reporting *(⚠️ DISPUTED 2026-08-11 — "post-FEC" is a mislabel: `lib/src/congestioncontrol.c:44` computes `reported_lost = lost + recovered / CONGESTION_FEC_RECOVERED_LOSS_DIVISOR`, adding FEC-recovered units to reported loss rather than subtracting them; the code's own comment at `:41-43` contradicts itself. Practical impact was ~zero in the PR #236 validation logs (`fec_recovered` totals 15 and 6, rounds to 0 nearly every tick), so the bitrate-stability result below is unaffected — only the "post-FEC" framing is wrong. GH issue to be filed for the accounting-comment fix.)* (4128b99a, v0.1.844+). Fixes PS5 bitrate floor-throttling via outcome-aware accounting; also fixes audio race, frameprocessor flushed flag, alloc_frame stale counters. ✅ Hardware A/B 2026-08-11: A arm bitrate dip 5825k→5691k recovers to 5825k; B arm 5825k flat. Pre-fix staircase (5825k→3579k) eliminated.
- **Opened, Hardware Validation Pending (PR #236):** Takion ENOBUFS `select()` retry-chain hardening (3cd6223d, v0.1.898). Closes the gap PR #209 left open — `select()` in the recv path only retried `EINTR`, so transient ENOBUFS still killed the recv thread (`CHIAKI_EVENT_QUIT code=11`), reproduced 2/2 on hardware logs `36193250515`/`36347840658`. Adds `CHIAKI_ERR_NETWORK_TRANSIENT` retry classification, doubles `sceNetInit` pool 4→8 MiB with measured justification, and fixes feedback-sender throttle (170/s → 125/s active, ~5/s idle; `FEEDBACK_STATE_TIMEOUT_MIN_MS`=8 kept as a deliberate latency/rate tradeoff). Pass condition: zero code=11 quits over a ≥5 min LAN session. See `TODO.md` "In Review" item 6.
...
- **Opened, Hardware Validation Pending (PR #256):** Frame-delivery-pattern probe for GH #251 (c8e2c553, v0.1.940). Diagnostics only, no behaviour change. GH #251 (filed 2026-08-12) established the felt "input lag" is video lag — input, decode, display, and arrival-to-display all measure clean on the client, so the delay is console→Vita delivery; the open question is whether the console paces its byte sends unevenly (H1) or the Wi-Fi link smears them (H2). Log `86770605157` (healthy RSSI median 74) showed `cadence_max`/`cadence_min` tightly clustered across 92 windows, which looks quantised rather than smeared, but the existing min/max/avg stats over ~31 samples/s can't distinguish one outlier per second from a repeating bimodal pattern. This PR adds, at the existing `lib/src/videoreceiver.c` frame-boundary sample: a 9-bucket inter-frame gap histogram (the discriminator), fps-scaled cumulative frame-index drift with skip/resync accounting, and gap-vs-previous-frame-size attribution with an adaptive large-frame threshold — emitted as a new `PIPE/DELIVERY` line once/sec, `PIPE/STAGE` untouched, zero new clock reads. Unit-verified (7 assertions; `./tools/build.sh test` only compiles for target hardware, so the assertions were additionally re-run natively on the host); `senior-code-guardian` approved with two non-blocking cosmetic findings. Filed three unrelated recv-path follow-ups while mapping (#253 malloc/free per video packet in `gkcrypt.c`, #254 mislabeled/under-counting malloc-burst instrumentation, #255 extra per-packet `select()` syscall) — none plausibly explain the 300-1000ms gaps, deferred. **Not yet validated on hardware** — next step is a ≥3 min capture read against the PR body's decision table, verdict posted to #251. See `TODO.md` "In Review" items 7-8.
- **Current Focus:** Hardware capture for PR #256 (GH #251 delivery-pattern verdict). Follow-up issues #219/#220/#221/#222/#224/#225/#226/#253/#254/#255 prioritized behind it. PR #227 fast_restart validation queued for next degraded-stream episode. Residual known lag = intrinsic Wi-Fi jitter (#219/#206 remnants) — unchanged.
- **Root-Cause Model (Validated 2026-08-11):**
  - **PS5 bitrate ratchet three-factor INPUT model:**
    1. Loss reports (input) — ✅ Fixed by PR #213 (post-FEC accounting — ⚠️ mislabeled, `congestioncontrol.c:44` **adds** `recovered/4` rather than subtracting; see `TODO.md:220`)
  ...
  - **Original GH #206 Jitter Investigation (Narrowed 2026-08-10):**
    ...
    - **⚠️ DISPUTED (2026-08-11):** The "jitter 50–90ms" / "genuine Wi-Fi arrival-time jitter" readings above come from `vita/src/host_metrics.c:325-334`'s `RTT = base + jitter`, where `base` (`session.rtt_us`) is set once by Senkusha and never updated, and `jitter` is a control-channel-only inter-arrival EWMA (`takion.c:1985-2012`) — video/audio never touch it. That's exactly what a control-cadence EWMA yields regardless of signal quality, explaining why the proximity move to RSSI 100 changed nothing. Not deleted — the receive-queue-drain and PS5-ratchet evidence above have independent support — but the "genuine Wi-Fi jitter" framing of this specific number is unproven. Corrective fix planned: PR B, branch `fix/latency-metric-truthfulness`, gated on PR #236 hardware validation.
  - **Remaining scope:** GH #219/#220/#221/#222/#224/#225/#226 follow-up investigation; latency-metric truthfulness fix (PR B) and PS5 ratchet recovery-rate tuning (both new 2026-08-11, GH issues to be filed)
...
| Date | Update | Owner |
|------|--------|-------|
| 2026-08-12 | **PR #256 Opened — Frame-Delivery-Pattern Probe for GH #251 (v0.1.940, code c8e2c553)** — GH #251 established the felt "input lag" is video lag: every client-side stage (input, decode, display, arrival-to-display) measures clean, so the delay is console→Vita delivery. Left open whether the console paces byte sends unevenly (H1) or the Wi-Fi link smears them (H2) — log `86770605157` (healthy RSSI median 74) showed `cadence_max`/`cadence_min` tightly clustered across 92 windows (looks quantised), but min/max/avg over ~31 samples/s can't distinguish one outlier/s from a repeating bimodal pattern. Adds a 9-bucket inter-frame gap histogram, fps-scaled cumulative frame-index drift, and gap-vs-previous-frame-size attribution at the existing frame-boundary sample in `lib/src/videoreceiver.c`, emitted as a new `PIPE/DELIVERY` line once/sec; `PIPE/STAGE` byte-identical, zero new clock reads, cost < 2µs/boundary. 7 unit tests (host-run, since `./tools/build.sh test` only compiles for target); `senior-code-guardian` approved with two non-blocking cosmetic findings. Filed three unrelated recv-path follow-ups while mapping (#253, #254, #255), none plausibly explain the 300-1000ms gaps. **Status:** diagnostics only, no behaviour change; built and unit-verified, hardware validation PENDING — next step is a ≥3 min hardware capture read against the PR's decision table, verdict posted to #251. | @mauricio-gg |
| 2026-08-11 | **PR #236 Opened — Takion ENOBUFS `select()` Retry-Chain Hardening (v0.1.898, code 3cd6223d)** — Root cause: PR #209 hardened `recv()` against transient ENOBUFS but left `select()` retrying only `EINTR`, so the same errno still killed the recv thread (`CHIAKI_EVENT_QUIT code=11`), reproduced 2/2 on hardware logs `36193250515` (LAN) and `36347840658` (Internet), build `9fa39cd3156d`. Fix adds `CHIAKI_ERR_NETWORK_TRANSIENT` classification with retry policy owned by `takion_recv()`. Also: `sceNetInit` pool 4→8 MiB (measured, logged), feedback-sender throttle fix (170/s → 125/s active, ~5/s idle; `FEEDBACK_STATE_TIMEOUT_MIN_MS`=8 kept deliberately to avoid up to 8ms added input latency). **Also flagged two disputed findings while reviewing this branch's evidence** (both corrected in place, not deleted): (1) the "genuine Wi-Fi jitter ~60ms" conclusion from 2026-08-10 reads a control-channel-only EWMA + a Senkusha-handshake-once RTT, not a live measurement — see Epic 4 root-cause model above; (2) PR #213's "post-FEC" framing is a mislabel — `congestioncontrol.c:44` adds recovered loss to reported loss rather than subtracting it (near-zero practical impact in these logs). Both filed as deferred work; GH issues to be filed. **Status:** Hardware validation PENDING — pass condition is zero code=11 quits over a ≥5 min LAN session. | @mauricio-gg |
