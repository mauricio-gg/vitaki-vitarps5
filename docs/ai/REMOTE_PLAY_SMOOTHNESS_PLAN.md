...
- **PS5's congestion response is a downward-only ratchet, not a threshold trip.** Confirmed via chiaki-ng's `packet_loss_max` clamp (commit [`2a0dd5f`](https://github.com/streetpea/chiaki-ng/commit/2a0dd5f)) and our own PR #213 (GH #206) evidence: reported loss in the 0.8-5% range was enough to drive `target_bitrate` down in a staircase (5825k → 2591k over 75s) with no mid-session recovery — it doesn't take >10% to trigger throttling, any nonzero reported loss ratchets it down further. The 10% cap exists to bound how hard a single burst can drive the ratchet, not to define a "safe" reporting threshold below it. Do NOT over-report; we already cap, and PR #213 moved reporting to post-FEC so recovered losses stop counting at all.
