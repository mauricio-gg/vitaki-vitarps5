...
static void test_settings_streaming_item_invariants(void) {
  ...
  assert(UI_SETTINGS_ITEM_QUALITY_FALLBACK_POLICY == 2);
  assert(UI_SETTINGS_ITEM_FPS_TARGET == 3);
  assert(UI_SETTINGS_ITEM_FORCE_30_FPS == 4);
  assert(UI_SETTINGS_ITEM_AUTO_DISCOVERY == 5);
  assert(UI_SETTINGS_ITEM_SHOW_LATENCY == 6);
  assert(UI_SETTINGS_ITEM_SHOW_NETWORK_ALERTS == 7);
  assert(UI_SETTINGS_ITEM_SHOW_STREAM_EXIT_HINT == 8);
  assert(UI_SETTINGS_ITEM_CLAMP_SOFT_RESTART_BITRATE == 9);
  assert(UI_SETTINGS_ITEM_FILL_SCREEN == 10);
  assert(UI_SETTINGS_ITEM_SHOW_NAV_LABELS == 11);
  assert(UI_SETTINGS_ITEM_CIRCLE_BUTTON_CONFIRM == 12);
  assert(UI_SETTINGS_STREAMING_ITEM_COUNT == 13);
}
