    ...
    docker run --rm \
        ...
        bash -c "
            ...
            if command -v cppcheck >/dev/null 2>&1; then
                eval \"cppcheck --enable=warning,performance,portability --std=c99 --language=c --inline-suppr -q --suppressions-list=.cppcheck-suppressions -I vita/include -I lib/include '-DLOGD(...)=' '-DLOGE(...)=' vita/src/ vita/include/\${STRICT}\"
            else
