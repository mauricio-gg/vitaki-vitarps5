...
static void utf16_to_utf8_local(const SceWChar16 *src, size_t src_max, char *dst, size_t dst_size) {
  size_t i = 0;
  size_t o = 0;
  if (!dst || dst_size == 0)
    return;
...
static void open_psn_auth_code_ime(void) {
  ...
  param.maxTextLength =
      (sizeof(profile_auth_ime_input_buf) / sizeof(profile_auth_ime_input_buf[0])) - 1;
...
static void poll_psn_auth_code_ime(uint64_t now_unix) {
  ...
  utf16_to_utf8_local(profile_auth_ime_input_buf,
                      sizeof(profile_auth_ime_input_buf) / sizeof(profile_auth_ime_input_buf[0]),
...
static void draw_profile_card(int x, int y, int width, int height, bool selected) {
  ...
  ui_draw_circle(icon_x + icon_size / 2, icon_y + icon_size / 2, icon_size / 2,
...
static void draw_connection_info_card(int x, int y, int width, int height, bool selected) {
  ...
  const char *auth_status = psn_auth_state_label_for(psn_state, now_unix);
  uint32_t auth_color = RGBA8(0xF4, 0x43, 0x36, 255);
...
UIScreenType ui_screen_draw_profile(void) {
  ...
  if (assist_panel_h > 0 &&
      (login_active || profile_state.current_section == PROFILE_SECTION_CONNECTION)) {
