...
void ui_cards_update_cache(bool force_update) {
  ...
  if (num_hosts > 1) {
    ...
    for (int i = 0; i < num_hosts; i++)
      if (temp_cards[i].is_registered)
        sorted[out++] = temp_cards[i];
    for (int i = 0; i < num_hosts; i++)
      if (!temp_cards[i].is_registered)
        sorted[out++] = temp_cards[i];
...
static void update_card_focus_animation(int new_focus_index) {
  ...
  if (card_focus_anim.focused_card_index >= 0 && card_focus_anim.focus_start_us > 0) {
    ...
    if (progress >= 1.0f) {
      ...
      card_focus_anim.current_scale =
          ui_lerp(CONSOLE_CARD_FOCUS_SCALE_MIN, CONSOLE_CARD_FOCUS_SCALE_MAX, eased);
    }
  } else {
    card_focus_anim.current_scale = CONSOLE_CARD_FOCUS_SCALE_MIN;
  }
...
static float get_card_scale(int card_index, bool is_focused) {
  ...
  if (card_index == card_focus_anim.previous_focused_card_index &&
      card_focus_anim.unfocus_start_us > 0) {
    uint64_t now_us = sceKernelGetProcessTimeWide();
    float elapsed_ms = (float)(now_us - card_focus_anim.unfocus_start_us) / 1000.0f;
    float progress = elapsed_ms / (float)CONSOLE_CARD_FOCUS_DURATION_MS;
...
void ui_cards_render_single(ConsoleCardInfo *console, int x, int y, bool selected,
  ...
  if (selected && !is_unpaired) {
    // 2px outline glow
    ui_draw_rounded_rect(draw_x - 2, draw_y - 2, card_w + 4, card_h + 4, 14,
                         CONSOLE_CARD_GLOW_COLOR);
  }
  ...
  ui_draw_rounded_rect(draw_x + shadow_offset, draw_y + shadow_offset, card_w, card_h, 12,
                       shadow_color);
  ...
  int name_bar_y =
      draw_y + card_h -
      (int)(CARD_NAME_BAR_BOTTOM_OFFSET * scale);  // Position name bar near bottom of card
  int name_bar_padding = (int)(12 * scale);
  ui_draw_rounded_rect(draw_x + name_bar_padding, name_bar_y, card_w - name_bar_padding * 2,
  ...
  if (is_cooldown_card) {
    ...
    if (console->status == 0)
      status_tex = ellipse_green;
    else if (console->status == 1)
      status_tex = ellipse_red;
    else if (console->status == 2)
...
void ui_cards_render_grid(void) {
  ...
  if (cooldown_active) {
    ...
    sceClibSnprintf(banner_text, sizeof(banner_text),
                    "Streaming stopped: %s - Please wait a few moments", reason);
  ...
  if (drag_active) {
    ...
    if (anim_offset < 0.0f)
      anim_offset = 0.0f;
    if (anim_offset > max_anim)
      anim_offset = max_anim;
  }
  ...
  for (int i = draw_start; i < draw_end; i++) {
    ...
    bool card_cooldown = cooldown_host && card_cache.cards[i].host == cooldown_host;
    float scale = get_card_scale(i, selected);

    ui_cards_render_single(&card_cache.cards[i], card_x, card_y, selected, card_cooldown, scale);
  }
  ...
  if (num_cards > CARDS_VISIBLE_MAX) {
    ...
    if (total_pages > 1) {
      ...
      for (int p = 0; p < total_pages; p++) {
        uint32_t dot_color =
            (p == current_page) ? UI_COLOR_PRIMARY_BLUE : RGBA8(120, 120, 120, 150);
