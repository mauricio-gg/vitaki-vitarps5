...
const char *host_quit_reason_label(ChiakiQuitReason reason) {
  switch (reason) {
    ...
    case CHIAKI_QUIT_REASON_CTRL_UNKNOWN:
      return "Control channel failure";
    ...
    case CHIAKI_QUIT_REASON_CTRL_CONNECTION_REFUSED:
      return "Control connection refused";
...
void host_update_disconnect_banner(const char *reason) {
  ...
  sceClibSnprintf(context.stream.disconnect_reason, sizeof(context.stream.disconnect_reason), "%s",
                  reason);
