...
uint32_t host_saturating_add_u32_report(uint32_t lhs, uint32_t rhs, const char *counter_name,
...
LossDetectionProfile host_loss_profile_for_mode(VitaChiakiLatencyMode mode) {
  LossDetectionProfile profile = {.window_us = LOSS_EVENT_WINDOW_DEFAULT_US,
...
void host_adjust_loss_profile_with_metrics(LossDetectionProfile *profile) {
  ...
  float target_mbps = (float)host_latency_mode_target_kbps(context.config.latency_mode) / 1000.0f;
  ...
  if (bitrate_known) {
    if (measured_mbps <= target_mbps * 0.85f) {
      profile->event_threshold = host_clamp_u32(profile->event_threshold + 1, 1, 6);
      profile->min_frames = host_clamp_u32(profile->min_frames + 1, 2, 8);
      profile->frame_threshold = host_clamp_u32(profile->frame_threshold + 2, 4, 24);
      profile->burst_frame_threshold = host_clamp_u32(profile->burst_frame_threshold + 1, 3, 16);
  ...
  if (measured_fps && clamp_target && measured_fps <= clamp_target) {
    profile->event_threshold = host_clamp_u32(profile->event_threshold + 1, 1, 6);
    profile->frame_threshold = host_clamp_u32(profile->frame_threshold + 1, 4, 24);
    profile->burst_frame_threshold = host_clamp_u32(profile->burst_frame_threshold + 1, 3, 16);
  }
}
